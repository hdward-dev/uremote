...
public sealed class DesktopControllerSession
{
    ...
    public async Task RunAsync(LoginState state, string target, string ffmpeg, CancellationToken ct, int captureType = 1)
    {
        ...
        try
        {
            ...
            var room = await api.JoinDeviceAsync(target, ct); joined = true;
            ...
            }, TimeSpan.FromSeconds(20), ct, [ControllerProtocol.ConnectOptions(state.DeviceId, captureType)]);
        ...
        {
            stop.Cancel(); peer = null;
            foreach (var decoder in decoders.Values) await decoder.DisposeAsync();
            if (joined) { using var deadline = new CancellationTokenSource(TimeSpan.FromSeconds(5)); try { await api.LeaveDeviceAsync(target, deadline.Token); } catch { } }

...
{
    public static byte[] ConnectOptions(string deviceId, int captureType = 1, bool assistance = false, bool enableClipboard = false) => Join(Int(1, (ulong)captureType), Int(2, ulong.MaxValue),
        ...
        Int(8, 4), Text(9, deviceId), Int(10, assistance ? 2UL : 1UL),
        Blob(11, Join(Int(1, 2), Int(2, 1), Int(3, 2), enableClipboard ? Int(8, 3) : [])), Text(12, "4.41.0"));
    ...
    public static byte[] Echo(ulong seq, bool enableClipboard = false) => Envelope(seq, 3, Join(Text(2, "{\"seq\":" + seq + "}"),
        Blob(4, enableClipboard ? Join(Int(1, 2), Int(2, 2), Int(3, 2)) : Int(3, 2))));
